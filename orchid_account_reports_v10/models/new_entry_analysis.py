...
class OrchidAccountEntryAnalysisNew(models.Model):
    _name = "orchid.account.entry.analysis.new"
    _description = "Orchid Account Entry Analysis New"
    _auto = False
    _rec_name = 'date'
    _order = 'date desc'
    ...
    division_id = fields.Many2one('orchid.account.division',string='Division')
    move_id = fields.Many2one('account.move',string="Journal Entry")

    # 
    # def init(self):
    #     cr = self.env.cr
    #     tools.drop_view_if_exists(cr, 'orchid_account_entry_analysis_new')
    #     cr.execute("""
    #         create or replace view orchid_account_entry_analysis_new as (
    #             SELECT
    #             	mvl.id,
	...
	# 				mvl.orchid_div_id as division_id,
	# 				mvl.orchid_br_id as branch_id,
	# 				mvl.amount_residual,
	# 				mvl.write_date,
	# 				mvl.move_id,
	# 				mvl.payment_id,
